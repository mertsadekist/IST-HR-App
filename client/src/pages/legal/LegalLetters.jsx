...
import { composeWithLetterhead, htmlToPdfBlob, downloadBlob } from '@utils/pdf';
...
export default function LegalLetters() {
  ...
  // letterhead when one is set — and opens it in a new tab to print.
  const handlePrint = async (letter) => {
    const win = window.open('', '_blank'); // open synchronously to keep the user gesture
    try {
      const rtl = isArabic(letter.rendered_html || letter.content);
      const lh = letterLh(letter);
      let blob;
      if (lh) {
        const res = await getLetterheadBytes(lh.companyId);
        blob = await composeWithLetterhead({
          letterheadBytes: res.data, letterheadType: lh.type,
          html: buildLetterHtml(letter, { bare: true }), rtl, marginsMm: lh.margins,
        });
      } else {
        blob = await htmlToPdfBlob(buildLetterHtml(letter, { bare: false }), { rtl });
      }
      const url = URL.createObjectURL(blob);
      if (win) win.location.href = url; else downloadBlob(blob, `${letter.template_name || 'letter'}.pdf`);
    } catch (err) {
      if (win) win.close();
        ...
        {viewLetter && (
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <Button variant="secondary" onClick={() => handlePrint(viewLetter)}><Printer size={14} /> {t('common.print', 'Print')}</Button>
