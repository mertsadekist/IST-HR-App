...
export default function CareersJob() {
  ...
  useEffect(() => {
    (async () => {
      ...
      catch (e) { setError(apiErr(e, t('careers.not_available'))); }
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!form.first_name || !form.last_name || !form.email || !form.phone) { setError(t('careers.required_fields')); return; }
    ...
    try {
      ...
      setStep(4);
    } catch (err) { setError(apiErr(err, t('careers.submit_error'))); }
      ...
      <div className="max-w-3xl mx-auto px-5 py-6">
        {step === 1 && (
          <div className="space-y-5">
            {job.is_closed && <div className="p-3 rounded-xl bg-amber-50 text-amber-700 text-sm">{t('careers.deadline_passed')}</div>}
            <Section title={t('careers.about')} body={job.description} />
            <Section title={t('careers.responsibilities')} body={job.responsibilities} />
            <Section title={t('careers.requirements')} body={job.qualifications} />
            <Section title={t('careers.required_skills')} body={job.required_skills} />
            <Section title={t('careers.preferred_skills')} body={job.preferred_skills} />
            {job.languages && <Section title={t('careers.languages')} body={job.languages} />}
            {job.benefits && <Section title={t('careers.benefits')} body={job.benefits} />}
        ...
        {step === 2 && (
          <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
            <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-2"><ShieldCheck size={18} style={{ color: brand }} /> {t('careers.privacy_title')}</h2>
            <p className="text-sm text-slate-600 leading-relaxed">{t('careers.privacy_body', { company: job.company_name })}</p>
            <label className="flex items-start gap-2.5 cursor-pointer text-sm text-slate-700">
              ...
              {t('careers.privacy_accept')}
            </label>
            <div className="flex gap-2">
              <button onClick={() => setStep(1)} className="px-4 py-2 rounded-xl text-sm bg-slate-100 text-slate-600">{t('careers.back')}</button>
              <button disabled={!consent} onClick={() => setStep(3)} className="px-5 py-2 rounded-xl text-sm text-white font-semibold disabled:opacity-40" style={{ background: brand }}>{t('careers.continue')}</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <form onSubmit={submit} className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
            <h2 className="text-lg font-semibold text-slate-800">{t('careers.your_application')}</h2>
            {error && <div className="p-2.5 rounded-lg bg-red-50 text-red-600 text-sm">{error}</div>}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label={t('careers.f_first_name')} req value={form.first_name} onChange={(v) => set('first_name', v)} />
              <Field label={t('careers.f_last_name')} req value={form.last_name} onChange={(v) => set('last_name', v)} />
              <Field label={t('careers.f_email')} req type="email" value={form.email} onChange={(v) => set('email', v)} />
              <Field label={t('careers.f_phone')} req value={form.phone} onChange={(v) => set('phone', v)} />
              <Field label={t('careers.f_location')} value={form.current_location} onChange={(v) => set('current_location', v)} />
              <Field label={t('careers.f_nationality')} value={form.nationality} onChange={(v) => set('nationality', v)} />
              <Field label={t('careers.f_current_title')} value={form.current_job_title} onChange={(v) => set('current_job_title', v)} />
              <Field label={t('careers.f_years')} type="number" value={form.years_experience} onChange={(v) => set('years_experience', v)} />
              <Field label={t('careers.f_expected_salary')} value={form.expected_salary} onChange={(v) => set('expected_salary', v)} />
              <Field label={t('careers.f_notice')} value={form.notice_period} onChange={(v) => set('notice_period', v)} />
            ...
            <div>
              <label className="text-xs font-semibold text-slate-600">{t('careers.cv_label')}</label>
              <label className="mt-1 flex items-center gap-2 border-2 border-dashed border-slate-200 rounded-xl px-3 py-3 cursor-pointer hover:border-violet-400">
                ...
                <span className="text-sm text-slate-500">{cv ? cv.name : t('careers.cv_upload')}</span>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setStep(2)} className="px-4 py-2 rounded-xl text-sm bg-slate-100 text-slate-600">{t('careers.back')}</button>
              <button type="submit" disabled={submitting} className="px-6 py-2.5 rounded-xl text-sm text-white font-semibold disabled:opacity-50 inline-flex items-center gap-2" style={{ background: brand }}>
                {submitting && <Loader2 size={15} className="animate-spin" />} {t('careers.submit')}
              </button>
            </div>
          </form>
        )}
        ...
        {step === 4 && (
          <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center">
            ...
            <h2 className="text-xl font-bold text-slate-800">{t('careers.success_title')}</h2>
            <p className="text-slate-600 mt-2 max-w-md mx-auto">{t('careers.success_body', { title: job.title, company: job.company_name })}</p>
          </div>
        )}
      </div>
